...
export const OPServerOrderPlacing = () => {
    ...
    function fnCreateBench(responceData) {
        const benchlst = responceData
    ...
    function fnSaveSelectedButton() {
        ...
        const newContent = tbl_menu.current.tableData;
        const roomId = getAppStoreData.room;
        const SeverName = getAppStoreData.username;
        socketRef.current.emit('updateServerOrderPlacing', docId, newContent, roomId, SeverName);
    ...
    function handleGroupButtonClick(groupBtnName, index) {
        ...
        switch (groupBtnName) {
            case "beanchlst":
                ...
                tbl_menu.current.grpbtnrowid = ctlBench.current.groupbtndata[index].rowid;
    ...
    useEffect(() => {
        ...
        socketRef.current.emit('joinServerOrderPlacing', getAppStoreData.room);

        // Listen for the 'menuList' event from the server
        socketRef.current.on('sendServerOrderPlacing', (responseData) => {
            initControl(responseData);
        });

        socketRef.current.on('documentUpdated', (docId, newContent, status) => {
            const index = ctlBench.current.groupbtndata.findIndex(item => item.rowid == docId);

            ctlBench.current.groupbtndata[index].status = status;
            ctlBench.current.groupbtndata[index].btn_values = newContent;
            fnCreateBench(ctlBench.current.groupbtndata);
                            ...
                            <div className="modal-dialog modal-dialog-centered ">
                                <div className="modal-content" style={{ height: "400px" }}>
                                    <div className="modal-header">
                                        <h5 className="modal-title">Order Pls</h5>
                                        <OPButton
