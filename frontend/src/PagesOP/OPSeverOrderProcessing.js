...
export const OPServerOrderProcessing = () => {
    ...
    function fnCreateBench(responceData) {
        const benchlst = responceData
            .filter(sts => sts.status == "processing" || sts.status == "waiting")
            .map(item => ({
                rowid: item._id,
    ...
    function fnSaveSelectedButton() {
        ...
        socketRef.current.emit('updateServerOrderProcessing', docId, newContent, roomId);
    ...
    useEffect(() => {
        ...
        socketRef.current.emit('joinServerOrderProcessing', getAppStoreData.room);
